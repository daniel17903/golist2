...
type DbWrite =
  | { kind: "insert-shared-list"; id: string; name: string; createdByDeviceId: string }
  | { kind: "insert-list-item"; id: string; listId: string; name: string; createdByDeviceId: string }
  | { kind: "insert-share-token"; tokenId: string; listId: string; createdByDeviceId: string };
...
const queryMock = vi.fn(async (text: string, values: unknown[] = []) => {
  ...
  if (
    ...
    const listId = asString(values[0], "listId");
    const list = sharedLists.get(listId);
    return toResult(list ? [{ id: list.id, created_by_device_id: list.createdByDeviceId }] : []);
  }

  if (text.includes("INSERT INTO shared_lists(id, name, created_by_device_id, created_at, updated_at) VALUES ($1, $2, $3, NOW(), NOW())")) {
  ...
  if (text.includes("SELECT id, name, created_at, updated_at FROM shared_lists WHERE id = $1 LIMIT 1")) {
    ...
    const list = sharedLists.get(listId);
    return toResult(
      list ? [{ id: list.id, name: list.name, created_at: list.createdAt, updated_at: list.updatedAt }] : [],
    );
  }
  ...
  if (text.includes("INSERT INTO share_tokens(id, list_id, created_by_device_id, created_at)")) {
    ...
    return toResult([{ created_at: new Date().toISOString() }]);
  }
...
describe("frontend/backend integration via playwright", () => {
  ...
  runE2E("creates a list and persists shared list records", async () => {
  ...
  runE2E("upserts an item from the UI and persists list_items", async () => {
    const { chromium } = await import("playwright");
    const browser = await chromium.launch();
    ...
    try {
      ...
      const itemWrite = dbWrites.find((entry): entry is Extract<DbWrite, { kind: "insert-list-item" }> =>
        entry.kind === "insert-list-item",
      );
      expect(itemWrite?.name).toBe("Milch");
      expect(itemWrite ? isUuid(itemWrite.id) : false).toBe(true);
      expect(itemWrite ? isUuid(itemWrite.createdByDeviceId) : false).toBe(true);
    } finally {
      await browser.close();
    }
  });

  runE2E("creates a share token and persists share_tokens", async () => {
    const { chromium } = await import("playwright");
    const browser = await chromium.launch();
    ...
    try {
      ...
      const listWrite = dbWrites.find((entry): entry is Extract<DbWrite, { kind: "insert-shared-list" }> =>
        entry.kind === "insert-shared-list",
      );
      ...
      }, { listId: listWrite?.id ?? "", deviceId: listWrite?.createdByDeviceId ?? "" });

      await expect.poll(() => dbWrites.filter((entry) => entry.kind === "insert-share-token").length).toBe(1);

      const tokenWrite = dbWrites.find((entry): entry is Extract<DbWrite, { kind: "insert-share-token" }> =>
        entry.kind === "insert-share-token",
      );
      expect(tokenWrite ? isUuid(tokenWrite.tokenId) : false).toBe(true);
      expect(tokenWrite ? isUuid(tokenWrite.listId) : false).toBe(true);
      expect(tokenWrite ? isUuid(tokenWrite.createdByDeviceId) : false).toBe(true);
    } finally {
      await browser.close();
    }
  });
