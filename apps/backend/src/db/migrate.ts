...
// Arbitrary, fixed advisory lock key shared by every `runMigrations()` call.
// It serializes concurrent migration runs (e.g. two cold-start serverless
// instances racing at once) so only one can apply migrations at a time,
// closing the check-then-act race between the SELECT/INSERT below.
const MIGRATION_ADVISORY_LOCK_KEY = 733_182_193
